...
/*
    Desafio Tetris Stack – Nivel Mestre
    -----------------------------------
    Este código implementa:

    - Fila circular fixa de 5 peças
    - Pilha de reserva com capacidade 3
    - Operações avançadas:
        * jogar peça (dequeue)
        * reservar (push)
        * usar peça reservada (pop)
        * trocar frente com topo
...
typedef struct {
    Peca data[FILA_CAP];   // Armazena as peças
    int head;              // Índice do início da fila
    int tail;              // Índice do final
    int count;             // Quantas peças existem
    int next_id;           // Usado para gerar IDs automaticamente
} Fila;

/* Estrutura da pilha linear */
typedef struct {
    Peca data[PILHA_CAP];  // Armazena as peças reservadas
    int top;               // Índice do topo
    int count;             // Total armazenado
} Pilha;

/* Inicializa a fila e já preenche com 5 peças aleatórias */
void iniciar_fila(Fila *f) {
    ...
    // Enche a fila com 5 peças iniciais
